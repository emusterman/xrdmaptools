...
def timed(func):
    def wrapper(*args, **kwargs):
        ...
        toc(output=False, print_msg=True)
        return out
    return wrapper
...
# pair_casting = timed(pair_casting)
...
def pair_casting_index_best_grain(all_ref_qs,
    ...
    if len(pairs) > 0:        
        ...
        connections = [[np.nan,] * len(all_spot_qs)]
        qofs = [np.nan,] * len(all_spot_qs)
...
def multiple_seed_casting(seeds,
    ...
    for i, seed in iterate(seeds):
        ...
        indexing_results.append(indexing)
        qofs.append(qof)

        # Find and exclude included pairs
        if exclude_found_seeds:
            print('Checking future pairs')
    ...
    # Sort by qof
    if sort_results:
        indexing_results = [x for _, x in sorted(zip(qofs,
                                                     indexing_results),
                                            key=lambda pair: pair[0],
                                            reverse=True)]
        qofs = sorted(qofs, reverse=True)
                
    return indexing_results, np.asarray(qofs)
...
def pattern_decomposition_from_seeds(start_seeds,
    ...
    indexing, qofs = _internal_indexing(start_seeds, all_spot_qs)
    # indexing = np.array(indexing, dtype=object)

    # Mask out not useful indexing. May switch to qof_minimum
    mask = np.array([len(ind) > 1 for ind in indexing])
    indexing = [ind for ind, b in zip(indexing, mask) if b]
    # indexing = indexing[mask]
    qofs = np.asarray(qofs)[mask]
    included_indexing_mask = included_indexing_mask[mask]
    
    # All seeds happen to be excluded
    if len(indexing) < 1:
        return best_indexing, np.asarray(best_qofs)

    # Iteratively decompose pattern based on seeds
    iter_count = 0
    while True:
        # print(iter_count)
        # print([len(ind) for ind in indexing])

        # Find and record best results
        best_ind = np.nanargmax(qofs)
        best_indexing.append(indexing[best_ind].astype(int))
        best_qofs.append(qofs[best_ind])

        # Remove already indexed spots from further analysis
        excluded_spots.update(set(indexing[best_ind][:, 0]))
        included_spot_mask[np.array(list(excluded_spots))] = False
        valid_mask = np.asarray([True,] * len(indexing))
        recalc_mask = np.asarray([False,] * len(indexing))
        
        for i in range(len(indexing)):
            indexed_spots = set(indexing[i][:, 0])
        ...
        # indexing = indexing[valid_mask]
        indexing = [ind for ind, b in zip(indexing, valid_mask) if b]
        ...
        if recalc_mask.sum() > 0:
            ...
            if ORIENTATION_SEEDS:
                ...
                # recalc_seeds = [indexing[idx] for idx in idxs] # from initial indexing
            else:
                recalc_seeds = []
            ...
            for idx, new_indexed in zip(recalc_mask.nonzero()[0], new_indexing):
                indexing[idx] =  new_indexed
            qofs[recalc_mask] = new_qofs

            # Remove invalid connections again
            if ORIENTATION_SEEDS:
                ...
                if np.any(~new_valid_mask):
                    ...
                    indexing = [ind for ind, b in zip(indexing, new_valid_mask) if b]
                    # indexing = indexing[new_valid_mask]
                    qofs = qofs[new_valid_mask]
                    recalc_mask = recalc_mask[new_valid_mask]
        
        # Conditionals to kill iteration
        iter_count += 1
        if (len(indexing) < 1 # Nothing left to compare
